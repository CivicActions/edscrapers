...
def domains_table():
    return dash_table.DataTable(
        ...
        data=df.groupby('domain', as_index=False).mean().round({'weighted score': 2}).to_dict('records'),
        sort_action='native',
...
def publishers_table():
    return dash_table.DataTable(
        ...
        data=df.groupby('publisher', as_index=False).mean().round({'weighted score': 2}).to_dict('records'),
        sort_action='native',
        style_cell={'textAlign': 'left', 'whiteSpace': 'normal'},
        # virtualization=True,
        style_cell_conditional=[
