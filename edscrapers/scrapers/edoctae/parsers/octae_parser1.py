...
def parse(res) -> dict:
    ...
    for container in dataset_containers:
        ...
        for resource_link in page_resource_links:
            ...
            # concatenate the parents with class 'headersLevel1' & 'headersLevel2'
            resource['description'] = str(resource_link.\
                                       find_parent(class_='contentText').find_previous_sibling(class_='headersLevel1').contents[0]).strip() +\
                                        " - " + str(resource_link.\
                                       find_parent(class_='contentText').find_previous_sibling(class_='headersLevel2').contents[1]).strip()
